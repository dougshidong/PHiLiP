        ...
        if (nstate_input == 1) {
            if (limiter_type == limiter_enum::none) {
                ...
                if (nstate_input == dim + 2)
                    return std::make_unique< PositivityPreservingLimiter_Zhang2010<dim, 1, real> >(parameters_input);
                else {
                ...
                if (nstate_input == dim + 2)
                    return std::make_unique< PositivityPreservingLimiter_Wang2012<dim, 1, real> >(parameters_input);
                else {
                    std::cout << "Error: Cannot create Positivity-Preserving limiter for nstate_input != dim + 2" << std::endl;
                    std::abort();
                }
            ...
            if (limiter_type == limiter_enum::none) {
                ...
                if (nstate_input == dim + 2)
                    return std::make_unique< PositivityPreservingLimiter_Zhang2010<dim, 2, real> >(parameters_input);
                else {
                    std::cout << "Error: Cannot create Positivity-Preserving limiter for nstate_input != dim + 2" << std::endl;
                    std::abort();
                }
                ...
                if (nstate_input == dim + 2)
                    return std::make_unique< PositivityPreservingLimiter_Wang2012<dim, 2, real> >(parameters_input);
                else {
                    std::cout << "Error: Cannot create Positivity-Preserving limiter for nstate_input != dim + 2" << std::endl;
                    std::abort();
                }
            ...
            if (limiter_type == limiter_enum::none) {
                ...
                if (nstate_input == dim + 2)
                    return std::make_unique< PositivityPreservingLimiter_Zhang2010<dim, 3, real> >(parameters_input);
                else {
                    std::cout << "Error: Cannot create Positivity-Preserving limiter for nstate_input != dim + 2" << std::endl;
                    std::abort();
                }
                ...
                if (nstate_input == dim + 2)
                    return std::make_unique< PositivityPreservingLimiter_Wang2012<dim, 3, real> >(parameters_input);
                else {
                    std::cout << "Error: Cannot create Positivity-Preserving limiter for nstate_input != dim + 2" << std::endl;
                    std::abort();
                }
            ...
            if (limiter_type == limiter_enum::none) {
                ...
                if (nstate_input == dim + 2)
                    return std::make_unique< PositivityPreservingLimiter_Zhang2010<dim, 4, real> >(parameters_input);
                else {
                    std::cout << "Error: Cannot create Positivity-Preserving limiter for nstate_input != dim + 2" << std::endl;
                    std::abort();
                }
                ...
                if (nstate_input == dim + 2)
                    return std::make_unique< PositivityPreservingLimiter_Wang2012<dim, 4, real> >(parameters_input);
                else {
                    std::cout << "Error: Cannot create Positivity-Preserving limiter for nstate_input != dim + 2" << std::endl;
                    std::abort();
                }
            ...
            if (limiter_type == limiter_enum::none) {
                ...
                if (nstate_input == dim + 2)
                    return std::make_unique< PositivityPreservingLimiter_Zhang2010<dim, 5, real> >(parameters_input);
                else {
                    std::cout << "Error: Cannot create Positivity-Preserving limiter for nstate_input != dim + 2" << std::endl;
                    std::abort();
                }
                ...
                if (nstate_input == dim + 2)
                    return std::make_unique< PositivityPreservingLimiter_Wang2012<dim, 5, real> >(parameters_input);
                else {
                    std::cout << "Error: Cannot create Positivity-Preserving limiter for nstate_input != dim + 2" << std::endl;
                    std::abort();
                }
            ...
            if (limiter_type == limiter_enum::none) {
                ...
                if (nstate_input == dim + 2)
                    return std::make_unique< PositivityPreservingLimiter_Zhang2010<dim, 6, real> >(parameters_input);
                else {
                    std::cout << "Error: Cannot create Positivity-Preserving limiter for nstate_input != dim + 2" << std::endl;
                    std::abort();
                }
                ...
                if (nstate_input == dim + 2)
                    return std::make_unique< PositivityPreservingLimiter_Wang2012<dim, 6, real> >(parameters_input);
                else {
                    std::cout << "Error: Cannot create Positivity-Preserving limiter for nstate_input != dim + 2" << std::endl;
                    std::abort();
                }
