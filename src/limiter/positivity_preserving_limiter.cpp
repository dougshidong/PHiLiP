    ...
    const dealii::hp::QCollection<1>                        oneD_quadrature_collection)
{
    ...
    for (auto soln_cell : dof_handler.active_cell_iterators()) {
        ...
        real theta = get_density_scaling_value<dim, nstate, double>(soln_cell_avg[0], local_min[0], pos_eps, p_avg);
        ...
        if (nstate == dim + 2) {
            ...
            std::vector<real> theta2 = get_theta2_Zhang2010<dim, nstate, double>(p_lim, soln_cell_avg, soln_at_q, n_quad_pts, pos_eps, euler_physics->gam);
        ...
        write_limited_solution<dim, nstate, double>(solution, soln_dofs, n_shape_fns, current_dofs_indices);
    }
}
