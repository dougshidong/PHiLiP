...
BurgersLimiter<dim, nstate>::BurgersLimiter(const PHiLiP::Parameters::AllParameters *const parameters_input)
...
int BurgersLimiter<dim, nstate>::run_test() const
{
    ...
    for (unsigned int igrid = igrid_start; igrid < n_grids; igrid++) {
           ...
           // grid->refine_global(log2(17)); //make sure shock is not on the face
