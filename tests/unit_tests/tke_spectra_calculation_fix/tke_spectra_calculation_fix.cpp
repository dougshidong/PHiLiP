...
int main (int argc, char * argv[])
{
    ...
    std::unique_ptr<PHiLiP::FlowSolver::FlowSolver<dim,nspecies,nstate>> flow_solver = PHiLiP::FlowSolver::FlowSolverFactory<dim,nspecies,nstate>::select_flow_case(&all_parameters, parameter_handler);
    ...
    PHiLiP::SetInitialCondition<dim,nspecies,nstate,double>::read_values_from_file_and_project(flow_solver->dg,input_filename_prefix);
    pcout << "done." << std::endl;

    // create the PeriodicTurbulence object
    std::unique_ptr<PHiLiP::FlowSolver::PeriodicTurbulence<dim, nspecies, nstate>> periodic_turbulence = std::make_unique<PHiLiP::FlowSolver::PeriodicTurbulence<dim,nspecies,nstate>>(&all_parameters);
