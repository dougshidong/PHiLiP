...
{
    relaxation_parameter = compute_relaxation_parameter_implicit(dt);
...
real RRKExplicitODESolver<dim,real,n_rk_stages,MeshType>::compute_relaxation_parameter_implicit(real &dt) const
{
    ...
    double gamma_kp1; 
    const double conv_tol = 2.22E-13;
    int iter_counter = 0;
    const int iter_limit = 1000;
    if (use_secant){

        const double initial_guess_0 = 1.0;// - 1E-5;
        const double initial_guess_1 = 1.0 + 1E-5;
        double residual = 1.0;
