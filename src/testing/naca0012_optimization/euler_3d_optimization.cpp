...
const unsigned int nx_des_var_start = 10;//20;
const unsigned int nx_des_var_end   = 40;//100;
const unsigned int nx_des_var_step  = 10;//20;

const int max_design_cycle = 300;
...
::run_test () const
{
    ...
    for (unsigned int poly_degree = POLY_START; poly_degree <= POLY_END; ++poly_degree) {
        for (unsigned int n_des_var = nx_des_var_start; n_des_var <= nx_des_var_end; n_des_var += nx_des_var_step) {
...
::optimize (const unsigned int nx_ffd, const unsigned int poly_degree) const
{
    ...
    if (grid_type == GridType::naca0012) {
        ...
        std::shared_ptr<HighOrderGrid<dim,double>> naca0012_mesh = read_gmsh <dim, dim> ("naca0012_hopw_ref1.msh");
    ...
    if (optimization_problem_type == OptimizationProblemType::drag_minimization) {
        ...
        lift_target = lift_functional.evaluate_functional() * 1.05;
        moment_target = moment_functional.evaluate_functional() * 1.0;
        volume_target = volume_functional.evaluate_functional() * 1.0;
    }
    if (grid_type == GridType::cylinder) {
