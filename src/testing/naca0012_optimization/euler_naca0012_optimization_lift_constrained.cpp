...
::optimize (const unsigned int nx_ffd, const unsigned int level) const
{
    ...
    if constexpr (dim == 2) {
        ...
        ffd_origin = dealii::Point<dim> (0.0, 0.0, -0.061);
        ffd_rectangle_lengths = std::array<double,dim> {{0.999, 3.1, 0.122}};
            //ffd_rectangle_lengths = std::array<double,dim> {{1.0,0.122}};

        ffd_ndim_control_pts = {{nx_ffd, 10, 3}};
    ...
    if constexpr (dim == 2) {
        ...
        for (unsigned int i_ctl = 0; i_ctl < ffd.n_control_pts; ++i_ctl) {
            ...
            for (unsigned int d_ffd = 0; d_ffd < dim; ++d_ffd) {
                ...
                if (   ijk[0] == 0 // Constrain first column of FFD points.
                    || ijk[0] == ffd_ndim_control_pts[0] - 1  // Constrain last column of FFD points.
                    //|| ijk[2] == 1 // Constrain middle row of FFD points.
                    || d_ffd == 0 // Constrain x-direction of FFD points.
                    || d_ffd == 1 // Constrain y-direction of FFD points.
                ) {
                    continue;
                }
                ++n_design_variables;
    ...
    if (grid_type == GridType::naca0012) {
        ...
        if (dim==2) {
            ...
            std::shared_ptr<HighOrderGrid<dim,double>> naca0012_mesh = read_gmsh <dim, dim> ("naca0012_hopw_ref"+std::to_string(param.grid_refinement_study_param.num_refinements)+".msh",true);
	    ...
	    //for (unsigned int i = 0; i < param.grid_refinement_study_param.num_refinements; ++i) {
	    //    naca0012_mesh->refine_global();
	    //}
