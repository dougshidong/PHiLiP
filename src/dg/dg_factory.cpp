    ...
    const std::shared_ptr<Triangulation> triangulation_input)
{
    ...
    if (parameters_input->use_weak_form) {
        if (pde_type == PDE_enum::advection) {
            return std::make_shared< DGWeak<dim,1,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::advection_vector) {
            return std::make_shared< DGWeak<dim,2,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::diffusion) {
            return std::make_shared< DGWeak<dim,1,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::convection_diffusion) {
            return std::make_shared< DGWeak<dim,1,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::burgers_inviscid) {
            return std::make_shared< DGWeak<dim,dim,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::burgers_viscous) {
            return std::make_shared< DGWeak<dim,dim,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::burgers_rewienski) {
            return std::make_shared< DGWeak<dim,dim,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::euler) {
            return std::make_shared< DGWeak<dim,dim+2,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
            ...
            return std::make_shared< DGWeak<dim,dim+3,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        ...
        if (pde_type == PDE_enum::advection) {
            return std::make_shared< DGStrong<dim,1,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::advection_vector) {
            return std::make_shared< DGStrong<dim,2,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::diffusion) {
            return std::make_shared< DGStrong<dim,1,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::convection_diffusion) {
            return std::make_shared< DGStrong<dim,1,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::burgers_inviscid) {
            return std::make_shared< DGStrong<dim,dim,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::burgers_viscous) {
            return std::make_shared< DGStrong<dim,dim,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::burgers_rewienski) {
            return std::make_shared< DGStrong<dim,dim,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::euler) {
            return std::make_shared< DGStrong<dim,dim+2,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::navier_stokes) {
            return std::make_shared< DGStrong<dim,dim+2,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if ((pde_type == PDE_enum::physics_model) && (model_type == Model_enum::reynolds_averaged_navier_stokes) && (rans_model_type == RANSModel_enum::SA_negative)) {
            return std::make_shared< DGStrong<dim,dim+3,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        } else if (pde_type == PDE_enum::p_poisson) {
            return std::make_shared< DGStrong<dim,1,real,MeshType> >(parameters_input, sub_parameters_input, degree, max_degree_input, grid_degree_input, triangulation_input);
        }
#if PHILIP_DIM==3
...
::create_discontinuous_galerkin(
    ...
    const std::shared_ptr<Triangulation> triangulation_input)
{
    return create_discontinuous_galerkin(parameters_input, nullptr, degree, max_degree_input, grid_degree_input, triangulation_input);
}
