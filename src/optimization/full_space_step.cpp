    ...
    //const PHiLiP::Parameters::LinearSolverParam & param = )
{
    ...
    switch(solver_type) {
        ...
        case gmres: {
            ...
            const bool 	use_default_residual = true; // default: true
    ...
    Constraint<Real> &equal_constraints)
{
    ...
    //     std::cout << "linear residual after solve_KKT_system: " << residual.l2_norm() << std::endl;
    ...
    AlgorithmState<Real> &algo_state )
{
    ...
    //     rhs2->axpy(-1.0,*lhs2);
    //     std::cout << "rhs1->norm() before solve " << rhs1->norm() << std::endl;
    //     std::cout << "rhs2->norm() before solve " << rhs2->norm() << std::endl;
    //     MPI_Barrier(MPI_COMM_WORLD);
    //     // Reset rhs
    //     // rhs1 is the negative gradient of the Lagrangian
    //     computeLagrangianGradient(*rhs1, design_variables, lagrange_mult, *objective_gradient, equal_constraints);
    ...
    std::cout << "Finished solving augmented system..." << std::endl;
    ...
    penalty_value_ = computeAugmentedLagrangianPenalty(
        ...
        *adjoint_jacobian_lagrange,
        equal_constraints,
        penalty_offset);
    penalty_value_ = std::max(1e-1/lagrangian_gradient->norm(), 1.0);
    std::cout 
    ...
    /* Perform line-search */
    Real merit_function_value = merit_function_->value(design_variables, tol );
    step_state->nfval = 0;
    step_state->ngrad = 0;
    std::cout 
    ...
    AlgorithmState<Real> &algo_state )
{
    ...
    secant_->updateStorage(design_variables, *current_reduced_gradient, *previous_reduced_gradient_, *control_search_direction, search_norm, algo_state.iter+1);
...
std::string FullSpace_BirosGhattas<Real>::printName( void ) const
{
  std::stringstream hist;
  //hist << name;
  hist << "Biros and Ghattas' Full-space method with P2 preconditioner" << std::endl;;
