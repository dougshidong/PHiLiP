...
ErrorPostSampling<dim, nstate>::ErrorPostSampling(const Parameters::AllParameters *const parameters_input,
...
template <int dim, int nstate>
Parameters::AllParameters ErrorPostSampling<dim, nstate>::reinitParams(std::string path) const{
    // Copy all parameters
...
template <int dim, int nstate>
bool ErrorPostSampling<dim, nstate>::getSnapshotParamsFromFile() const{
...
template <int dim, int nstate>
bool ErrorPostSampling<dim, nstate>::getROMParamsFromFile() const{
    bool file_found = false;
    rom_points(0,0);
    std::string path = all_parameters->reduced_order_param.path_to_search; //Search specified directory for files containing "solutions_table"
...
template <int dim, int nstate>
void ErrorPostSampling<dim, nstate>::getROMPoints() const{
...
template <int dim, int nstate>
int ErrorPostSampling<dim, nstate>::run_test() const
{
...
#if PHILIP_DIM==1
        template class ErrorPostSampling<PHILIP_DIM, PHILIP_DIM>;
#endif

#if PHILIP_DIM!=1
        template class ErrorPostSampling<PHILIP_DIM, PHILIP_DIM+2>;
#endif
} // Tests namespace
} // PHiLiP namespace
