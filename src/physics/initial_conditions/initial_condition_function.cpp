...
// ========================================================
template <int dim, int nstate, typename real>
InitialConditionFunction_MultiSpecies_HighTemperature_VortexAdvection<dim,nstate,real>
...
template <int dim, int nstate, typename real>
real InitialConditionFunction_MultiSpecies_HighTemperature_VortexAdvection<dim,nstate,real>
::primitive_value(const dealii::Point<dim,real> &point, const unsigned int istate) const
{
    // Note: This is in non-dimensional form (free-stream values as reference)
    real value = 0.;
...
template <int dim, int nstate, typename real>
inline real InitialConditionFunction_MultiSpecies_HighTemperature_VortexAdvection<dim, nstate, real>
::value(const dealii::Point<dim,real> &point, const unsigned int istate) const
{
    real value = 0.0;
    value = convert_primitive_to_conversative_value(point,istate);
    ...
    , mach_inf_sqr(mach_inf*mach_inf)
{
    ...
    this->multi_species_calorically_perfect_euler_physics = std::dynamic_pointer_cast<Physics::MultiSpeciesCaloricallyPerfect<dim,dim+2+2-1,double>>( // Note: modify this when you change the number of species. nstate == dim+2+(nspecies)-1
                Physics::PhysicsFactory<dim,dim+2+2-1,double>::create_Physics(&parameters_euler)); // Note: modify this when you change the number of species. nstate == dim+2+(nspecies)-1
}
template <int dim, int nstate, typename real>
...
// ========================================================
template <int dim, int nstate, typename real>
InitialConditionFunction_MultiSpecies_TwoDimnsional_VortexAdvection<dim,nstate,real>
...
template <int dim, int nstate, typename real>
inline real InitialConditionFunction_MultiSpecies_TwoDimnsional_VortexAdvection<dim, nstate, real>
::value(const dealii::Point<dim,real> &point, const unsigned int istate) const
{
    real value = 0.0;
    value = convert_primitive_to_conversative_value(point,istate);
...
// ========================================================
template <int dim, int nstate, typename real>
InitialConditionFunction_MultiSpecies_ThreeDimnsional_VortexAdvection<dim,nstate,real>
...
template <int dim, int nstate, typename real>
real InitialConditionFunction_MultiSpecies_ThreeDimnsional_VortexAdvection<dim,nstate,real>
::primitive_value(const dealii::Point<dim,real> &point, const unsigned int istate) const
{
    // Note: This is in non-dimensional form (free-stream values as reference)
    real value = 0.;
...
template <int dim, int nstate, typename real>
inline real InitialConditionFunction_MultiSpecies_ThreeDimnsional_VortexAdvection<dim, nstate, real>
::value(const dealii::Point<dim,real> &point, const unsigned int istate) const
{
    real value = 0.0;
    value = convert_primitive_to_conversative_value(point,istate);
...
template <int dim, int nstate, typename real>
real InitialConditionFunction_MultiSpecies_TaylorGreenVortex<dim,nstate,real>
::primitive_value(const dealii::Point<dim,real> &point, const unsigned int istate) const
{
    // Note: This is in non-dimensional form (free-stream values as reference)
    real value = 0.;
    ...
    Parameters::AllParameters const *const param)
{
    ...
    if (flow_type == FlowCaseEnum::taylor_green_vortex) {
        ...
        if constexpr (dim==1 && nstate==dim+2+2-1){ // TO DO: N_SPECIES, dim = 1, nstate = dim+2+3-1
            return std::make_shared<InitialConditionFunction_MultiSpecies_HighTemperature_VortexAdvection<dim,nstate,real> >(param);
        }
        ...
        if constexpr (dim==3 && nstate==dim+2+2-1){ // Note: modify this when you change the number of species. nstate == dim+2+(nspecies)-1
            return std::make_shared<InitialConditionFunction_MultiSpecies_ThreeDimnsional_VortexAdvection<dim,nstate,real> >(param);
        }    
