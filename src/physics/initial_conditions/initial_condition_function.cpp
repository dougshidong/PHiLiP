...
// ========================================================
template <int dim, int nspecies, int nstate, typename real>
InitialConditionFunction_LowDensity2D<dim,nspecies,nstate,real>
...
template <int dim, int nspecies, int nstate, typename real>
real InitialConditionFunction_LowDensity2D<dim, nspecies, nstate, real>
::primitive_value(const dealii::Point<dim, real>& point, const unsigned int istate) const
{
    real value = 0.0;
    ...
    Parameters::AllParameters const *const param)
{
    ...
    if (flow_type == FlowCaseEnum::taylor_green_vortex) {
        ...
        if constexpr (dim==2 && nstate==dim+2)  return std::make_shared<InitialConditionFunction_LowDensity2D<dim,nspecies,nstate,real> > (param);
        ...
        if constexpr (dim==2 && nstate==dim+2){ 
            return std::make_shared<InitialConditionFunction_AcousticWave_Air<dim,nspecies,nstate,real> >(param);
        }
    } else if (flow_type == FlowCaseEnum::acoustic_wave_species) {
        if constexpr (dim==2 && nstate==dim+2){ 
            return std::make_shared<InitialConditionFunction_AcousticWave_Species<dim,nspecies,nstate,real> >(param);
        }
        ...
        if constexpr (dim==1 && nstate==dim+2){ 
            return std::make_shared<InitialConditionFunction_Euler_VortexAdvection<dim,nspecies,nstate,real> >(param);
        }
        ...
        if constexpr (dim==2 && nstate==dim+2+nspecies-1){ // Note: modify this when you change the number of species. nstate == dim+2+(nspecies)-1
        ...
        if constexpr (dim==2 && nspecies==2 && nstate==dim+2+nspecies-1){ // Note: modify this when you change the number of species. nstate == dim+2+(nspecies)-1
        ...
        if constexpr (dim==3 && nspecies==2 && nstate==dim+2+nspecies-1){ // Note: modify this when you change the number of species. nstate == dim+2+(nspecies)-1
        ...
        if constexpr (dim==3 && nstate==dim+2+nspecies-1){ // Note: modify this when you change the number of species. nstate == dim+2+(nspecies)-1
...
template class InitialConditionFunction_EulerBase <PHILIP_DIM, PHILIP_SPECIES, PHILIP_DIM + 2, double>;
template class InitialConditionFunction_LowDensity2D <PHILIP_DIM, PHILIP_SPECIES, PHILIP_DIM+2, double>;
