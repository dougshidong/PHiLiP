...
void AssembleResidualSubsetOOA<dim,nstate>::advance_to_end_time(std::shared_ptr<FlowSolver::FlowSolver<dim,nstate>> flow_solver, const double end_time) const
{
    ...
    dealii::LinearAlgebra::distributed::Vector<double> f_u_n(flow_solver->dg->solution); // f (last solution)
        dealii::LinearAlgebra::distributed::Vector<double> f_u_n_1(flow_solver->dg->solution); // f (last solution)

    dealii::LinearAlgebra::distributed::Vector<double> f_u_tilde(flow_solver->dg->solution); // f( intermediate value)
        dealii::LinearAlgebra::distributed::Vector<double> f_u_tilde_1(flow_solver->dg->solution); // f( intermediate value)
    ...
    while (flow_solver->ode_solver->current_time < end_time) {
        ...
        flow_solver->dg->assemble_residual(false,false,false,0.0,10); // assemble on group ID 0
        ...
        //     }
        if(this->all_parameters->use_inverse_mass_on_the_fly){
            flow_solver->dg->apply_inverse_global_mass_matrix(flow_solver->dg->right_hand_side, f_u_n_1);
        } else{
            flow_solver->dg->global_inverse_mass_matrix.vmult(f_u_n_1, flow_solver->dg->right_hand_side);
        }
        ...
        flow_solver->dg->assemble_residual(false,false,false,0.0,10); // assemble on group ID 10
            ...
            // }
        if(this->all_parameters->use_inverse_mass_on_the_fly){
            flow_solver->dg->apply_inverse_global_mass_matrix(flow_solver->dg->right_hand_side, f_u_tilde_1);
        } else{
