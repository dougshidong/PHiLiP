    ...
    std::array<dealii::LinearAlgebra::distributed::Vector<double>,dim> &rhs_aux)
{
    ...
    const unsigned int grid_degree = this->high_order_grid->grid_degree;
    const unsigned int poly_degree = i_fele;

    const unsigned int n_metric_dofs_cell = high_order_grid->dof_handler_grid.get_fe(high_order_grid->grid_degree).dofs_per_cell;
    ...
    for (unsigned int iface=0; iface < dealii::GeometryInfo<dim>::faces_per_cell; ++iface) {
        ...
        else if (current_face->at_boundary() && current_cell->has_periodic_neighbor(iface))
        {
            ...
            if (!current_cell->periodic_neighbor_is_coarser(iface) && current_cell_should_do_the_work(current_cell, neighbor_cell)) 
            {
                ...
                const unsigned int grid_degree_ext = this->high_order_grid->grid_degree;    
...
void DGBase<dim,real,MeshType>::assemble_residual (const bool compute_dRdW, const bool compute_dRdX, const bool compute_d2R, const double CFL_mass)
{
    ...
    //const dealii::MappingQ<dim,dim> mapping(high_order_grid->grid_degree);
    //const dealii::MappingQGeneric<dim,dim> mapping(high_order_grid->grid_degree);
    const auto mapping = (*(high_order_grid->mapping_fe_field));
    ...
    const unsigned int init_grid_degree = high_order_grid->grid_degree;
...
void DGBase<dim,real,MeshType>::output_face_results_vtk (const unsigned int cycle, const double current_time)// const
{
    ...
    const int grid_degree = high_order_grid->grid_degree;
...
void DGBase<dim,real,MeshType>::output_results_vtk (const unsigned int cycle, const double current_time)// const
{
    ...
    const dealii::Mapping<dim> &mapping = (*(high_order_grid->mapping_fe_field));
    const unsigned int grid_degree = high_order_grid->grid_degree;
...
{   
    ...
    for (auto cell = dof_handler.begin_active(); cell!=dof_handler.end(); ++cell, ++metric_cell) {
        ...
        const unsigned int fe_index_curr_cell = cell->active_fe_index();
        const unsigned int curr_grid_degree   = high_order_grid->grid_degree;//in the future the metric cell's should store a local grid degree. currently high_order_grid dof_handler_grid doesn't have that capability
        ...
        //setup metric cell
        const dealii::FESystem<dim> &fe_metric = high_order_grid->dof_handler_grid.get_fe(high_order_grid->grid_degree);
        const unsigned int n_metric_dofs = high_order_grid->dof_handler_grid.get_fe(high_order_grid->grid_degree).dofs_per_cell;
