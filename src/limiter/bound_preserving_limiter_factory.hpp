...
public:
    ...
    static std::unique_ptr< BoundPreservingLimiter<dim,nspecies,real> > create_limiter(
        const Parameters::AllParameters *const parameters_input);

    /// Create a pointer to the limiter specified in prms
    static std::unique_ptr< BoundPreservingLimiter<dim, nspecies, real> > select_limiter(
        const Parameters::AllParameters* const parameters_input);
