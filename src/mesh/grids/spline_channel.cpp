    ...
    const double channel_height)
{
    ...
    const SplineManifold spline_manifold;

    // Warp grid to be a gaussian spline
    //dealii::GridTools::transform (&(SplineManifold::warp), grid);
...
dealii::Point<2,real> SplineManifold::mapping(const dealii::Point<2,real> &chart_point) const 
{
...
dealii::Point<2> SplineManifold::pull_back(const dealii::Point<2> &space_point) const {
...
dealii::Point<2> SplineManifold::push_forward(const dealii::Point<2> &chart_point) const 
{
    return mapping<double>(chart_point);
}

dealii::DerivativeForm<1,2,2> SplineManifold::push_forward_gradient(const dealii::Point<2> &chart_point) const
{
    using ADtype = Sacado::Fad::DFad<double>;
...
std::unique_ptr<dealii::Manifold<2,2> > SplineManifold::clone() const
{
    return std::make_unique<SplineManifold>();
}
