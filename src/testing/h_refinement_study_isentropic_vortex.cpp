...
// --------------------------------------------------------
        ...
        for (int refinement = 0; refinement < this->n_calculations; ++refinement){
            ...
            double expected_order = params_modified.flow_solver_param.poly_degree + 1;
            if(c_value > c_array[nb_c_value])
                expected_order -= 1;
        ...
        convergence_table_density.clear();
        convergence_table_pressure.clear();
    }// c ESFR range loop
    conv_tab_file.close();
    return testfail;
}
