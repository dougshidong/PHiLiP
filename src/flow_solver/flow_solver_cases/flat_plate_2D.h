...
protected:
    /// Length of free area upwind to the flat plate.
    const double free_length;

    /// Height of free area above the flat plate.
    const double free_height;

    /// Length of the flat plate.
    const double plate_length;

    /// Skewness of the meshes in the x direction for the free area.
    const double skewness_x_free;

    /// Skewness of the meshes in the x direction for the plate area.
    const double skewness_x_plate;

    /// Skewness of the meshes in the y direction.
    const double skewness_y;

    /// Number of cells per x direction for the grid in free area.
    const int number_of_subdivisions_in_x_direction_free;

    /// Number of cells per x direction for the grid in plate area.
    const int number_of_subdivisions_in_x_direction_plate;

    /// Number of cells per y direction for the grid.
