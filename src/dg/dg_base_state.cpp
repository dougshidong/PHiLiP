...
DGBaseState<dim, nstate, real, MeshType, sub_nstate>::DGBaseState(const Parameters::AllParameters *const parameters_input,
...
template <int dim, int nstate, typename real, typename MeshType, int sub_nstate>
real DGBaseState<dim, nstate, real, MeshType,sub_nstate>::evaluate_CFL(std::vector<std::array<real, nstate> > soln_at_q,
...
#define INSTANTIATE_DISTRIBUTED(r, data, index) \
    template class DGBaseState <PHILIP_DIM, index, double, dealii::parallel::distributed::Triangulation<PHILIP_DIM>,1>;
...
#define INSTANTIATE_SHARED(r, data, index) \
    template class DGBaseState <PHILIP_DIM, index, double, dealii::parallel::shared::Triangulation<PHILIP_DIM>,1>;
BOOST_PP_SEQ_FOR_EACH(INSTANTIATE_SHARED, _, POSSIBLE_NSTATE)

#define INSTANTIATE_TRIA(r, data, index) \
    template class DGBaseState <PHILIP_DIM, index, double, dealii::Triangulation<PHILIP_DIM>,1>;
