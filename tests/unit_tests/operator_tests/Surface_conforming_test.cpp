...
#if 0
pcout<<"Lets take a look at FE Q"<<std::endl;
for(unsigned int iquad=0; iquad<n_metric_dofs; iquad++){
for(unsigned int idof=0; idof<n_metric_dofs; idof++){
dealii::Point<dim> pt = vol_GLL.point(iquad); 
            const unsigned int istate = fe_metric.system_to_component_index(idof).first; 
pcout<<" feQ "<<fe_metric.shape_value_component(idof,pt,istate)<<" idof "<<idof<<" iquad "<<iquad<<" istate "<<istate<<std::endl;
}
}
...
//pcout<<"FACE NUMBER "<<iface<<" NEIGH IFACE "<<neighbor_iface<<std::endl;
        ...
        for (unsigned int idof = 0; idof< n_metric_dofs; ++idof) {
            ...
            const unsigned int istate = fe_metric.system_to_component_index(idof).first; 
            const unsigned int ishape = fe_metric.system_to_component_index(idof).second; 
            mapping_support_points_neigh[istate][ishape] = val; 
        }
#endif
...
//#endif
            ...
            for (unsigned int idof = 0; idof < n_metric_dofs; ++idof) {
                const unsigned int istate = fe_metric.system_to_component_index(idof).first; 
                const unsigned int ishape = fe_metric.system_to_component_index(idof).second; 
                for (unsigned int idof2 = 0; idof2 < n_metric_dofs; ++idof2) {
                    const unsigned int istate2 = fe_metric.system_to_component_index(idof2).first; 
                    const unsigned int ishape2 = fe_metric.system_to_component_index(idof2).second; 
                    if(istate == istate2 && istate == 0){
                        if(std::abs(mapping_support_points_neigh[istate2][ishape2] - mapping_support_points[istate][ishape])<1e-14)
                            if(std::abs(mapping_support_points_neigh[istate2+1][ishape2] - mapping_support_points[istate+1][ishape])<1e-14)
                                pcout<<" istate "<<istate<<" ishape "<<ishape<<" with neigh istate "<<istate2<<" ishape "<<ishape2<<std::endl;
                    }
                }
            }
#endif
    ...
    for(unsigned int iquad=0; iquad<n_quad_face_pts; iquad++){
        ...
        for(int idim=0; idim<dim; idim++){
            ...
            normal_phys_ext[iquad][idim] = 0.0;
            for(int jdim=0; jdim<dim; jdim++){
                normal_phys_int[iquad][idim] += metric_cofactor[iquad][idim][jdim] * unit_normal_int[jdim]; 
                normal_phys_ext[iquad][idim] += metric_cofactor_neigh[iquad][idim][jdim] * unit_normal_int[jdim]; 
            }
