...
AdvectionLimiter<dim, nstate>::AdvectionLimiter(
...
int AdvectionLimiter<dim, nstate>::run_test() const
{
    ...
    if (!all_parameters_new.limiter_param.use_OOA) {
        test_result = run_advection_lim();
    } else {
        test_result = run_advection_lim_conv();
    }
    return test_result; //if got to here means passed the test, otherwise would've failed earlier
}

template <int dim, int nstate>
int AdvectionLimiter<dim, nstate>::run_advection_lim() const
{
...
template <int dim, int nstate>
int AdvectionLimiter<dim, nstate>::run_advection_lim_conv() const
{
    PHiLiP::Parameters::AllParameters all_parameters_new = *all_parameters;
                ...
                for (unsigned int iquad = 0; iquad < n_quad_pts; ++iquad) {
                    ...
                    for (int istate = 0; istate < nstate; ++istate) {
                        ...
                        for (int idim = 0; idim < dim; idim++) {
                            uexact *= sin(2.0 * pi * (qpoint[idim] - adv_speeds[idim] * all_parameters_new.flow_solver_param.final_time));//for grid 1-3
                        }
