...
int main (int argc, char * argv[])
{
    ...
    std::array<double, dim+nspecies+1> soln;
    ...
    for (auto cell : grid.active_cell_iterators()) {
        for (unsigned int v=0; v < dealii::GeometryInfo<dim>::vertices_per_cell; ++v) {
            ...
            for (int s=0; s<nstate; s++) {
                soln[s] = real_gas_physics.manufactured_solution_function->value(vertex, s);
            }
            for (int d=0; d<dim; d++) {
                ...
                dealii::Tensor<2,nstate,double> jacobian_an = real_gas_physics.convective_flux_directional_jacobian (soln, normal);

                dealii::Tensor<2,nstate,double> jacobian_fd;
                for (int col=0; col<nstate; col++) {
                    // const double dw = perturbation*soln[col];
                    // std::array<double, dim+nspecies+1> soln_plus = soln;
                    // std::array<double, dim+nspecies+1> soln_mins = soln;
                    // soln_plus[col] += dw;
                    // soln_mins[col] -= dw;
                    // const std::array<dealii::Tensor<1,dim,double>,nstate> conv_flux_plus = real_gas_physics.convective_flux(soln_plus);
                    // const std::array<dealii::Tensor<1,dim,double>,nstate> conv_flux_mins = real_gas_physics.convective_flux(soln_mins);
                    ...
                    const double dw = perturbation*soln[col];
                    std::array<double, dim+nspecies+1> soln_plus2 = soln;
                    std::array<double, dim+nspecies+1> soln_plus = soln;
                    std::array<double, dim+nspecies+1> soln_mins = soln;
                    std::array<double, dim+nspecies+1> soln_mins2 = soln;
                    soln_plus2[col] += 2*dw;
                    soln_plus[col] += dw;
                    soln_mins[col] -= dw;
                    soln_mins2[col] -= 2*dw;

                    const std::array<dealii::Tensor<1,dim,double>,nstate> conv_flux_plus2 = real_gas_physics.convective_flux(soln_plus2);
                    const std::array<dealii::Tensor<1,dim,double>,nstate> conv_flux_plus = real_gas_physics.convective_flux(soln_plus);
                    const std::array<dealii::Tensor<1,dim,double>,nstate> conv_flux_mins = real_gas_physics.convective_flux(soln_mins);
                    const std::array<dealii::Tensor<1,dim,double>,nstate> conv_flux_mins2 = real_gas_physics.convective_flux(soln_mins2);
