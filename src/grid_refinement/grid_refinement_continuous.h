...
/** This class provides method to update the grid using a combination of continuous mesh and
  ...
  * is primarily done through GMSH for isotropic all-quad remeshing using the $L^\infty$ advancing front
  ...
  * mesh generator based on $L_p$-CVT energy minimization to produce an anisotropic all-quad mesh
  * have also been included.
  * 
  * Note: While there are some placeholder functions have been included and certain functionality support
  *       polynomial distributions, $p$ and $hp$ adaptation have not been fully implemented or tested.
  */ 
#if PHILIP_DIM==1
...
protected:
    ...
    /** For continuous methods, this function requires interfacing with
      ...
      * handled through GMSH for isotropic $L^\infty$ advancing front for
    ...
    /// Updates the global polynomial distribution based on the target $p$-field
    ...
    /** Placeholder function for inclusion of eventual $hp$ continuous mesh model
    ...
    /** For the current class this includes current and target h and p fields,
      * the anisotropic ratio and set magnitude of $p+1$ directional derivatives 
    ...
    /** Based on setup parameters, globally updates the target anisotropic mesh 
      ...
      * p_field. Together these describe the continuous $hp$-mesh model. This 
    ...
    /** For the $p=1$ case only, uses the hessian-based error estimate involving the
      ...
      * 
      * \f[
      *     B(\bm{x}) = (\operatorname{tr}{(|H_u|)})^{\frac{Lq}{2}}
      * \f]
      * 
      */ 
    void field_h_error();
    ...
    /** This method is based on the high-order continuous error model of Dolejsi et al. 2018
      ...
      *     \left(A_1(\bm{x}, p) A_2(\bm{x}, p)\right)^{\frac{q}{2}}
      * \f]
      * 
      * where the constant is ignored and $A_1$ and $A_2$ are the
    ...
    /** For use with the external Lp-CVT mesh generator, this function will generate
      ...
      * \f]
      * 
      * Where $V(x)$ is the passed to the Lp-CVT mesh generator and used in the node movement energy
    ...
    /** The continuous complexity usually represents an integral approximation
      ...
      * freedom from each element (for the varying $p$ case):
      * 
      * \f[
      *     C = \Sum_{i} {(p_i+1)^2}
      * \f]
