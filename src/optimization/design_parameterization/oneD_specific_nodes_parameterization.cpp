#include "oneD_specific_nodes_parameterization.hpp"
...
template<int dim>
OneDSpecificNodesParameterization<dim> :: OneDSpecificNodesParameterization(
...
template<int dim>
void OneDSpecificNodesParameterization<dim> :: store_prespecified_control_nodes()
{
    std::ifstream infile;
    std::string filepath;
    ...
    {
        std::cout<<"OneDSpecificNodesParameterization is only implemented for q1 and q2 grids. Aborting.."<<std::endl;
        std::abort();
    }

    infile.open(filepath);
    if(!infile) {
        std::cout << "Could not open file for OneDSpecificNodesParameterization."<< filepath << std::endl;
        std::abort();
    }
    ...
    const int n_nodes = control_nodes_list.size();
    control_slope.resize(n_nodes);
    const double PI = 4.0*atan(1.0);
    for(int i=0; i<n_nodes; ++i)
    {
        const double xval = control_nodes_list[i].first;
        const double yval = control_nodes_list[i].second;
        const double eta_val = (double)i/((double)(n_nodes-1));
        const double theta = 5.0*PI/12.0*(2.0*eta_val - 1.0);
        const double x_circle = -cos(theta);
        const double y_circle = sin(theta);
        control_slope[i] = (yval - y_circle)/(xval - x_circle);
    }
}

template<int dim>
void OneDSpecificNodesParameterization<dim> :: compute_control_index_to_vol_index()
{
    ...
    for(unsigned int i_vol = 0; i_vol<n_vol_nodes; ++i_vol) 
    {
        ...
        if(i_vol % dim == 0.0)
        {
            ...
            const int index = check_if_node_belongs_to_the_region(x,y);
            const bool is_part_of_region = (index >= 0);
            
            if(is_part_of_region)
            {
                is_a_control_node(i_vol) = 1;
                slopeval_global(i_vol) = control_slope[index];
            }
        }
    }
    ...
    slope_vals.reinit(control_index_range, control_ghost_range, this->mpi_communicator);  
    ...
    for(unsigned int i_vol = 0; i_vol < n_vol_nodes; ++i_vol)
    {
        ...
        if(is_a_control_node(i_vol) == 1)
        {
            ...
            slope_vals[count1] = slopeval_global(i_vol);
    ...
    slope_vals.update_ghost_values();
}

template<int dim>
void OneDSpecificNodesParameterization<dim> :: initialize_design_variables(VectorType &design_var)
{
...
template<int dim>
void OneDSpecificNodesParameterization<dim> :: compute_dXv_dXp(MatrixType &dXv_dXp) const
{
    const dealii::IndexSet &volume_range = this->high_order_grid->volume_nodes.get_partitioner()->locally_owned_range();
    ...
    for(unsigned int i_control=0; i_control<n_control_nodes; ++i_control)
    {
        ...
        if(volume_range.is_element(ivol))
        {
            ...
            const double slope = slope_vals(i_control);
...
template<int dim>
bool OneDSpecificNodesParameterization<dim> ::update_mesh_from_design_variables(
...
template<int dim>
unsigned int OneDSpecificNodesParameterization<dim> :: get_number_of_design_variables() const
{
    return n_control_nodes;
}

template<int dim>
int OneDSpecificNodesParameterization<dim> :: is_design_variable_valid(
    const MatrixType &dXv_dXp, 
...
template<int dim>
int OneDSpecificNodesParameterization<dim> :: check_if_node_belongs_to_the_region(const double x, const double y) const
{
    const double tol = 1.0e-5;
    const int n_nodes = control_nodes_list.size();
    for(int i=0; i<n_nodes; ++i)
    {
        if( sqrt(pow(x-control_nodes_list[i].first,2) + pow(y-control_nodes_list[i].second,2)) < tol )
        {
            return i;
        }
    }
