    ...
    for (auto pde = pde_type.begin(); pde != pde_type.end() && error == 0; pde++) {
        for (unsigned int poly_degree=1; poly_degree<3; ++poly_degree) {
            for (unsigned int igrid=2; igrid<5; ++igrid) {
                ...
                std::shared_ptr < DGBase<PHILIP_DIM, nspecies, double> > dg = DGFactory<PHILIP_DIM,nspecies,double>::create_discontinuous_galerkin(&all_parameters, poly_degree, grid);
                dg->allocate_system ();
